...
pub const Function = struct {
    ...
    fn allocAlignedLocal(f: *Function, ty: Type, _: CQualifiers, alignment: u32) !CValue {
        if (false) {
            // TODO: track whether a local is currently being used in any branch at all
            const target = f.object.dg.module.getTarget();
...
fn genBodyResolveState(f: *Function, inst: Air.Inst.Index, leading_deaths: []const Air.Inst.Index, body: []const Air.Inst.Index, inner: bool) error{ AnalysisFail, OutOfMemory }!void {
    ...
    // The exception to the above is allocs, which cannot be freed after the body is finished.
    // Remember how many there were so we can un-free any that were created in the body.
    ...
    // them. However, any local which was used as an alloc should no longer be free under any
    // circumstance.
    ...
    for (f.allocs.keys()[pre_allocs_len..]) |local_index| {
        const local = &f.locals.items[local_index];
        if (f.free_locals_map.getPtr(local.getType())) |locals_list| {
            // This local isn't free anymore because it's been used in an alloc
            _ = locals_list.swapRemove(local_index);
        }
    }
}
