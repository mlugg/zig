...
// TODO remove
pub fn getLoop(_: Liveness, _: Air.Inst.Index) LoopSlice {
    return .{ .deaths = &.{} };
}
...
fn analyzeInstLoop(
    ...
    switch (pass) {
        .loop_analysis => {
            var old_breaks = try data.breaks.clone(gpa);
            ...
            // Note: old_breaks is always a subset of breaks so this is correct
            const num_breaks = data.breaks.count() - old_breaks.count();
            ...
            while (it.next()) |key| {
                ...
                if (!old_breaks.contains(block_inst)) {
                    a.extra.appendAssumeCapacity(block_inst);
                    log.debug("[{}] %{}: includes break to {}", .{ pass, inst, block_inst });
                }
            }
